...
for i =1:length(xplot)
    ...
    Pe(i)=integral2(@(vx,vy)f_e_rec__(vx,vy,xplot(i),0).*vx.^2*v0^2,-3,3,-3,3,'AbsTol',1e10,'RelTol',1e-4)*params.me*10^6; %Prr
...
vPe= interp1(momentum{6,2},momentum{1,2},xplot,'spline');
vE = interp1(momentum{6,2},momentum{2,2},xplot,'spline');
% current
j=params.e*1e6*Ne.*((vE-vPi)-(vE+vPe))*1e3;
% hall term
hall=-j.*interp1(xnew,y(2,:),xplot,'spline')./Ne*params.B0*10^-9/params.e/10^6;
...
% divergence P term
divp=interp1(momentum{6,2},momentum{1,2}*1e3.*interp1(xnew,Bnew,momentum{6,2},'spline')*1e-9,xplot,'spline');
totalE=hall+convection+divp;
% temperature Trr  Tphiphi
Terr=interp1(momentum{6,2},momentum{7,2},xplot,'spline')./Ne/1e6/params.e;
Teff=interp1(momentum{6,2},momentum{8,2},xplot,'spline')./Ne/1e6/params.e;
%% save data
...
line_data{16,1}='j A/m^2';     line_data{16,2}=j;
line_data{17,1}='hall V/m';  line_data{17,2}=hall;
